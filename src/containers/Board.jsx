...
const Board = props => {
    const onDragEnd = result => {
        ...
        if (source.droppableId === destination.droppableId) { // Dropped in the same list
            props.reOrderList(source.droppableId, source.index, destination.index);
        } else { // Drop in other list
            props.moveCardToList(source.droppableId, draggableId, destination.droppableId, destination.index,);
        }
    };

    return (
        <div>
            <BoardContainer countColumns={
                ...
                <DragDropContext onDragEnd={onDragEnd}>
                    {
                        props.lists.map((list, listIndex) => (
                            <CardList key={
                                ...
                                list={list}
                                onChangeListName={
                                    listName => props.onChangeListName(listIndex, listName)
                                }
                                onRemoveList={
                                    () => props.onRemoveList(listIndex)
                                }
                                onChangeCardContent={
                                    (cardIndex, content) => props.onChangeCardContent(listIndex, cardIndex, content)
                                }
                                onAddCard={
                                    cardContent => props.onAddCard(listIndex, cardContent)
                                }
                                onRemoveCard={
                                    cardIndex => props.onRemoveCard(listIndex, cardIndex)
                                } />
                        ))
                    } </DragDropContext>
                <AddForm onConfirm={
...
const mapStateToProps = state => ({ lists: state.board.currentState.lists });
